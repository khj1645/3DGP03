...
void CStandardMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet)
{
	...
	pVertexBufferViews[nBuffers++] = m_d3dPositionBufferView; // Always has position

	if (m_pd3dTextureCoord0Buffer) pVertexBufferViews[nBuffers++] = m_d3dTextureCoord0BufferView;
	if (m_pd3dNormalBuffer) pVertexBufferViews[nBuffers++] = m_d3dNormalBufferView;
	if (m_pd3dTangentBuffer) pVertexBufferViews[nBuffers++] = m_d3dTangentBufferView;
	if (m_pd3dBiTangentBuffer) pVertexBufferViews[nBuffers++] = m_d3dBiTangentBufferView;
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	int nQuadsInWidth = nWidth - 1;
	int nQuadsInLength = nLength - 1;
	m_pnSubSetIndices[0] = nQuadsInWidth * nQuadsInLength * 4;
	m_ppnSubSetIndices[0] = new UINT[m_pnSubSetIndices[0]];

	for (int k = 0, z = 0; z < nQuadsInLength; z++)
	{
		for (int x = 0; x < nQuadsInWidth; x++)
		{
			int v0 = (z * nWidth) + x;
			int v1 = (z * nWidth) + (x + 1);
			int v2 = ((z + 1) * nWidth) + x;
			int v3 = ((z + 1) * nWidth) + (x + 1);
			m_ppnSubSetIndices[0][k++] = v0;
			m_ppnSubSetIndices[0][k++] = v1;
			m_ppnSubSetIndices[0][k++] = v2;
			m_ppnSubSetIndices[0][k++] = v3;
		}
	}
